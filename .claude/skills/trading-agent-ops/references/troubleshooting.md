...
| No `schedule` runs at all (only `workflow_dispatch` in the run list) | GitHub's cron is best-effort. On 2026-09-24 it never fired on the first day, although the workflow was `active` with a valid cron. | Claude routines "Trading agent: kick AM session / PM session / report" start `workflow_dispatch` runs at 9:45, 15:10 and 16:25 ET as a backup; the runner skips slots that already ran, so doubles are harmless. If a slot was still missed, check those routines with `list_triggers` (`last_run`). Don't run a missed morning session outside its window: the owner specified the first hour after the open. |
